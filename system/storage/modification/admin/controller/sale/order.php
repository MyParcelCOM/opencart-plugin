...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		//CIS
		$mp_status = "";
		$mp_shipping_info = $this->model_sale_order->get_shipment($result['order_id']);
		if($mp_shipping_info['squntity']>=$mp_shipping_info['quan']){
			$mp_status = 'Shipped';
		}
		else{
		...
		if (!empty($order_info)) {
			...
			foreach ($products as $product) {
				$data['order_products'][] = array(
					...
					'squantity'   => $product['shipped_product'], // CIS
					'price'      => $product['price'],
					'total'      => $product['total'],
  	...
  	if(!empty($orders)){
	    ...
	    foreach($items as $item) { 
	        ...
	        $total_weight = $total_weight-floatval($product_weight * $chk_qun);
  		    ...
  		    if($total_weight > $weight || $squantity==0 || $weight==0) {
			continue;
  		    }else{
		...
		continue;
  		...
  		if($shipping_code == 'my_parcel.my_parcel'){  			
			    ...
			    $insert = array();
			    if($shipmentId!=''){
				$update = array('order_id' => $order_id,'ship_id' => $shipmentId);
				$this->model_sale_order->myparcel_shipment_update($update);
				$this->model_sale_order->myparcel_shipment_history($update);
