...
class ControllerSaleMyParcel extends Controller
{
    ...
    public function setRecipient($order_id)
    {
        ...
        $street2                   = $order_info['shipping_address_2'];
        ...
        if ('GB' == $country_code) {
            $recipient->setStreet1($order_shipping_address_1)->setStreetNumber(221)->setCity(
                ...
                $phone
            )->setStreet2($street2)->setCompany($company);
        } else {
            $recipient->setStreet1($order_shipping_address_1)->setStreetNumber(221)->setCity(
                $order_shipping_city
            )->setPostalCode($order_shipping_postcode)->setFirstName($order_shipping_first_name)->setLastName(
                $order_shipping_last_name
            )->setCountryCode($country_code)->setEmail($order_billing_email)->setPhoneNumber($phone)->setStreet2($street2)->setCompany($company);
        }
        ...
        if (!empty($orders)) {
            ...
            foreach ($orders as $order_id) {
                ...
                if ($shipmentId != '') {
                    ...
                    $msg .= 'Order with OrderId '.$order_id.' has been exported successfully to MyParcel.com<br>';
                } else {
